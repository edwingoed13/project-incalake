...
export default defineNuxtConfig({
  ...
  nitro: {
    ...
    prerender: {
      // Deliberately EMPTY. Prerendering these pages made them plain static
      // files in .output/public, and a static file is not an ISR function:
      // Vercel serves it untouched until the next deploy, so on-demand purges
      // stopped working and admin edits (a blocked date, a price, a publish)
      // never reached the public site. Stale content is far worse than a
      // slower first load. Speed is handled by ISR + the hourly warmer.
      crawlLinks: false,
      routes: []
    }
  },
