...
<template>
  <div class="bg-white rounded-2xl shadow-sm p-4 sm:p-6 border border-slate-200">
    ...
    <form @submit="handleSubmit" class="space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          />
          <p v-if="errors.customer_first_name" class="mt-1 text-sm text-red-600">
        ...
        <div>
          ...
          />
          <p v-if="errors.customer_last_name" class="mt-1 text-sm text-red-600">
      ...
      <div v-if="pickupAvailable">
        ...
        />
        <p v-if="errors.pickup_location" class="mt-1 text-sm text-red-600">
      ...
      <div class="border-t border-slate-200 pt-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          >
            ...
            <span class="text-sm font-bold shrink-0" :class="paymentMethod === 'culqi' ? 'text-primary' : 'text-slate-700'">{{ t('checkout.card') }}</span>
            <div class="flex items-center gap-1 ml-auto">
              <span class="px-1.5 py-0.5 rounded text-[10px] font-black tracking-wide bg-white border border-slate-200 text-[#1a1f71]">VISA</span>
              <span class="px-1.5 py-0.5 rounded text-[10px] font-black tracking-wide bg-white border border-slate-200 text-[#eb001b]">MC</span>
              <span class="px-1.5 py-0.5 rounded text-[10px] font-black tracking-wide bg-white border border-slate-200 text-[#006fcf]">AMEX</span>
            </div>
          </label>
          <label
            class="flex items-center gap-3 px-3 py-2.5 border-2 rounded-xl cursor-pointer transition-all"
          ...
          >
            ...
            <span class="text-sm font-bold shrink-0" :class="paymentMethod === 'paypal' ? 'text-[#0070ba]' : 'text-slate-700'">PayPal</span>
      ...
      <div class="border-t border-slate-200 pt-4">
        <label class="flex items-start cursor-pointer">
          <input
            ...
            type="checkbox"
            class="mt-1 mr-3 w-5 h-5 text-primary border-slate-300 rounded focus:ring-primary"
          />
        ...
        <p v-if="errors.accepted_terms" class="mt-1 text-sm text-red-600">
    ...
    <AppModal v-model="showPoliciesModal" :title="modalTitle" max-width="max-w-2xl">
      <div v-for="(tour, index) in toursPolicies" :key="index" class="mb-6 last:mb-0">
        ...
        <div v-if="toursPolicies.length > 1" class="mb-3 pb-2 border-b border-slate-200">
        ...
        <div class="space-y-4">
          <div v-if="tour.policies">
            ...
            <div class="prose prose-sm max-w-none text-sm text-slate-700 bg-blue-50 p-3 rounded-lg border border-blue-200" v-html="sanitizeHtml(tour.policies)"></div>
          </div>
          ...
          <div v-if="tour.cancellationPolicy">
            ...
            <div class="prose prose-sm max-w-none text-sm text-slate-700 bg-red-50 p-3 rounded-lg border border-red-200" v-html="sanitizeHtml(tour.cancellationPolicy)"></div>
          </div>

          <!-- No policies configured for this tour -->
          <div v-if="!tour.policies && !tour.cancellationPolicy" class="text-sm text-slate-600 bg-slate-50 p-3 rounded-lg border border-slate-200">
