    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 md:pt-28 pb-16">
      ...
      <header class="mb-6 md:mb-8 flex items-end justify-between gap-4 flex-wrap">
        ...
        <button
          ...
          @click="confirmClear"
          type="button"
          class="text-xs font-bold text-slate-500 hover:text-red-500 transition-colors inline-flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-slate-100"
        >
      ...
      <div
        v-if="items.length === 0"
        class="bg-white rounded-2xl border border-slate-200 p-10 md:p-16 text-center"
      >
        <div class="mx-auto size-16 rounded-full bg-red-50 flex items-center justify-center mb-4">
          <HeartIcon class="size-8 text-red-400" aria-hidden="true" />
        </div>
        <h2 class="text-lg font-bold text-slate-800 mb-1">Tu lista está vacía</h2>
      ...
      <div v-else>
        ...
        <div v-if="items.length > 1" class="flex items-center justify-between gap-3 bg-white rounded-xl border border-slate-200 px-3 py-2 mb-3">
        ...
        <article
          v-for="item in items"
          :key="item.id"
          class="group relative bg-white rounded-xl border overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col"
          :class="isSelected(item.id) ? 'border-primary/50 ring-1 ring-primary/20' : 'border-slate-200'"
        >
          ...
          <NuxtLink :to="tourLink(item)" class="relative block aspect-[3/2] overflow-hidden bg-slate-100">
            <img
              v-if="item.image"
          ...
          <div class="p-3 flex-1 flex flex-col">
            <p v-if="item.city_slug" class="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">
              {{ item.city_slug }}
            </p>
            <NuxtLink :to="tourLink(item)" class="block">
              <h3 class="text-sm font-bold text-slate-900 group-hover:text-primary transition-colors line-clamp-2 leading-snug">
                {{ item.title || 'Tour' }}
              </h3>
            </NuxtLink>
            ...
            <div class="mt-auto pt-3 flex items-end justify-between gap-2">
              <div v-if="item.min_price" class="min-w-0 leading-tight">
                <span class="text-[10px] text-slate-400 font-medium block leading-none mb-0.5">Desde</span>
